...
@section('profileContent')
    ...
    @if($user->isLoggedProfile)
      ...
      <div class="row mb-4">
        <div class="col-2">
          ...
          <?php } else { ?>
            <img class="card-img-top" style="max-width:100px"src="<?php echo asset('uploads/avatars/' . $user->user->avatar);?>" alt="Card image cap">
          <?php }?>
        </div>
      ...
      <form method="post" action="{{URL::route('profile.update_profile')}}">
        <div class="form-group">
          <input type="date" name="born" value="{{$user->user->born}}">
        </div>

        <div class="form-group">
          <div class="custom-control custom-radio custom-control-inline">
            <input type="radio" name="sex" class="custom-control-input" value="male" id="radio-male" {{ $user->user->sex == 'male' ? 'checked' : '' }}>
            <label class="custom-control-label" for="radio-male">Masculino</label>
          </div>

          <div class="custom-control custom-radio custom-control-inline">
            <input type="radio" name="sex" class="custom-control-input" value="female" id="radio-female" {{$user->user->sex == 'female' ? 'checked' : '' }}>
            <label class="custom-control-label" for="radio-female">Feminino</label>
          </div>
        </div>
        <?php $locale = unserialize($user->user->locale) ?>
        <div class="form-row">
    ...
    @else
    @if($user->user->born != null)<b>Nascimento:</b> {{$user->user->born}}<br>@endif
    @if($user->user->sex != null)<b>Sexo:</b> {{$user->user->sex}}<br>@endif

    @if($user->user->locale != null)
    @php $locale = unserialize($user->user->locale) @endphp
    <b>Localidade:</b> {{$locale['city']}}, {{$locale['state']}} - {{$locale['country']}}<br>
    @endif

    <b>Entrou em:</b> {{$user->user->created_at}}<br>
    @endif
@endsection
